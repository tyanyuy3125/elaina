...
ELAINA_CALLABLE uint interleave_32bit(Vector2ui v) {
 ...
 * hashing utils
 ********************************************************/
ELAINA_CALLABLE Vector2ui blockCipherTEA(uint v0, uint v1, uint iterations = 16) {
	uint sum		 = 0;
...
ELAINA_CALLABLE uint64_t MurmurHash64A(const unsigned char *key, size_t len, uint64_t seed) {
...
ELAINA_CALLABLE uint64_t MixBits(uint64_t v);
...
template <typename T>
ELAINA_CALLABLE uint64_t HashBuffer(const T *ptr, size_t size, uint64_t seed = 0) {
	return MurmurHash64A((const unsigned char *) ptr, size, seed);
}

template <typename... Args> ELAINA_CALLABLE uint64_t Hash(Args... args);

template <typename... Args> ELAINA_CALLABLE void hashRecursiveCopy(char *buf, Args...);

template <> ELAINA_CALLABLE void hashRecursiveCopy(char *buf) {}

template <typename T, typename... Args>
ELAINA_CALLABLE void hashRecursiveCopy(char *buf, T v, Args... args) {
...
template <typename... Args> ELAINA_CALLABLE float HashFloat(Args... args) {
