...
public:
	...
	ELAINA_CALLABLE Matrix(typename std::enable_if_t<Rows == Cols, U> v) 
		: Eigen::Matrix<T, Rows, Cols, Options>(Vector<T, Rows>::Constant(v).asDiagonal()) {}

	template <typename OtherDerived>
	ELAINA_CALLABLE Matrix(const Eigen::MatrixBase<OtherDerived> &other)
		: Eigen::Matrix<T, Rows, Cols, Options>(other) {}

	template <typename OtherDerived>
	ELAINA_CALLABLE Matrix &operator=(const Eigen::MatrixBase<OtherDerived> &other) {
