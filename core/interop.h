...
namespace inter
{
    ...
    template <typename T>
    ELAINA_CALLABLE void swap(T &a, T &b)
    {
        T tmp = std::move(a);
        a = std::move(b);
        b = std::move(tmp);
    ...
    ELAINA_CALLABLE typename std::enable_if_t<sizeof(To) == sizeof(From) &&
    ...
    namespace span_internal
    {
        ...
        template <typename C>
        ELAINA_CALLABLE constexpr auto GetDataImpl(C &c, char) noexcept
            -> decltype(c.data())
        {
            return c.data();
        }
        ...
        template <typename C>
        ELAINA_CALLABLE constexpr auto GetData(C &c) noexcept -> decltype(GetDataImpl(c, 0))
        {
    ...
    public:
        ...
        template <size_t N>
        ELAINA_CALLABLE span(T (&a)[N]) : span(a, N) {}
        ELAINA_CALLABLE
        ...
        ELAINA_CALLABLE explicit span(V &v) noexcept : span(v.data(), v.size()) {}
        ...
        ELAINA_CALLABLE constexpr span(const V &v) noexcept : span(v.data(), v.size()) {}
    ...
    ELAINA_CALLABLE constexpr span<T> MakeSpan(T *ptr, size_t size) noexcept
    {
        return span<T>(ptr, size);
    }

    template <int &...ExplicitArgumentBarrier, typename T>
    ELAINA_CALLABLE span<T> MakeSpan(T *begin, T *end) noexcept
    {
    ...
    ELAINA_CALLABLE constexpr auto MakeSpan(C &c) noexcept
    ...
    ELAINA_CALLABLE constexpr span<T> MakeSpan(T (&array)[N]) noexcept
    {
        return span<T>(array, N);
    }

    template <int &...ExplicitArgumentBarrier, typename T>
    ELAINA_CALLABLE constexpr span<const T> MakeConstSpan(T *ptr, size_t size) noexcept
    {
        return span<const T>(ptr, size);
    }

    template <int &...ExplicitArgumentBarrier, typename T>
    ELAINA_CALLABLE span<const T> MakeConstSpan(T *begin, T *end) noexcept
    {
    ...
    template <int &...ExplicitArgumentBarrier, typename C>
    ELAINA_CALLABLE constexpr auto MakeConstSpan(const C &c) noexcept
        -> decltype(MakeSpan(c))
    {
        return MakeSpan(c);
    }
    ...
    template <int &...ExplicitArgumentBarrier, typename T, size_t N>
    ELAINA_CALLABLE constexpr span<const T> MakeConstSpan(const T (&array)[N]) noexcept
    {
    ...
    public:
        ...
        {
            ELAINA_CHECK(alloc == other.alloc); // TODO: handle this
            std::swap(ptr, other.ptr);
            std::swap(nAlloc, other.nAlloc);
            std::swap(nStored, other.nStored);
        }
    ...
    ELAINA_CALLABLE auto &get(tuple<T, Ts...> &t)
    {
        if constexpr (I == 0)
            return t.value;
        else
    ...
    template <size_t I, typename T, typename... Ts>
    ELAINA_CALLABLE const auto &get(const tuple<T, Ts...> &t)
    {
        if constexpr (I == 0)
            return t.value;
        else
