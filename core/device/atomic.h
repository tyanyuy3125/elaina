...
//#define ELAINA_LEGACY_CUDA_ATOMICS
...
public:
	...
	ELAINA_CALLABLE atomic(T val) { 
		store(val);
	}
	
	ELAINA_CALLABLE T load() const {
#ifdef ELAINA_DEVICE_CODE
#ifdef ELAINA_LEGACY_CUDA_ATOMICS
		return m_val;
#else
	...
	ELAINA_CALLABLE void store(const T& val) {
#ifdef ELAINA_DEVICE_CODE
#ifdef ELAINA_LEGACY_CUDA_ATOMICS
		m_val = val;
#else
	...
	ELAINA_CALLABLE operator T() const {
		return load();
	}
	
	ELAINA_CALLABLE T exchange(const T &val) {
#ifdef ELAINA_DEVICE_CODE
#ifdef ELAINA_LEGACY_CUDA_ATOMICS
	...
	ELAINA_CALLABLE T fetch_add(const T& val) {
#ifdef ELAINA_DEVICE_CODE
