...
void Logger::log(Level level, const string &msg, bool terminate)
{
    ...
    if (!sInitialized)
    {
        ...
        if (sAnsiControlSequence)
            std::cout << Log::TERMINAL_DEFAULT;
        sInitialized = true;
    }
    ...
    if (level <= sVerbosity)
    {
        ...
        if (sAnsiControlSequence)
            s += getLevelAnsiColor(level);
        if (sLogTime)
            s += Log::nowToString("%H:%M:%S ");

        s += getLogLevelString(level) + std::string(" ") + msg + "\n";

        if (sAnsiControlSequence)
            s += Log::TERMINAL_DEFAULT;
    ...
    if (level < Level::Error || terminate)
        exit(1);
}
